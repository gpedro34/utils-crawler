...
if(def.ipify.length > 0){
  def.ipify.forEach((el)=>{
    providers.push("https://geo.ipify.org/api/v1?apiKey="+el+"&ipAddress=*");
  });
}
...
const updateBlacklisted = () => {
  ...
  try{
    ...
    if(err.message.indexOf('Unexpected') >= 0){
      ...
      utils.logger('Unexpected Error trying to parse blacklisted Location Providers: ', err)
    }
  } finally {
    ...
    if(black.length > 0){
      black.forEach((el)=>{
        ...
        if(Math.abs(newDate - new Date(el.date)) > def.blacklistCooldown*60*1000){
