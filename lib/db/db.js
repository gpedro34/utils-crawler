...
class Checks {
	...
	async updateSSL(resSSL, id) {
		...
		try {
			...
			logger(
				'Errored updating checks table with ssl info where hash = ' + hash
			);
			logger('Query: ', query);
			logger('arr: ', arr);
			logger('Error: ', err);
					...
					utils.asyncForEach(oldIps, async el => {
						...
						query ='UPDATE checks '
									+'SET blocked = ?, '
											+'last_scanned = NOW() '
									+'WHERE hash = ? ';
						arr = [BLOCK_REASONS.OLD_IP, oldHash];
						await this.db.execute(query, arr);
						logger(
							'Peer ' + id + ' changed his IP from ' + el.ip + ' to ' + ip + '!'
						);
					});
				}
			}
	...
	async updatePub(resPub, peer) {
		...
		const query = 'UPDATE checks '
								 +'SET api = ? '
								 +'WHERE hash = ?';
		peer.hash = require('crypto')
			.createHash('md5')
			.update(peer.id + ',' + peer.ip)
			.digest('hex');
		try {
			await this.db.execute(query, [resPub.apiPort, peer.hash]);
			logger('Updated Public wallet details of peer with hash: ' + peer.hash);
		} catch (err) {
			logger(
				'Errored updating checks table with public wallet info where hash: ' +
					peer.hash
			);
			logger('Query: ', query);
			logger('Error: ', err);
	...
	async updateLocData(resLoc, peer, hash) {
		...
		try {
			...
			logger(
				'Updated Location data for ' + resLoc.country + ', ' + resLoc.city
			);
			this.updateLoc(resLoc, peer, hash);
		} catch (err) {
			logger(
				'Errored updating Location data for ' +
					resLoc.country +
					', ' +
					resLoc.city
			);
	...
	async updateLoc(resLoc, peer, hash) {
		...
		try {
			...
			await dbc.commit();
			logger('Updated location details of peer with ID: ' + peer.id);
		} catch (err) {
			logger(
				'Errored updating checks table with location info where peerId = ' +
					peer.id
			);
			logger('Query: ', query);
			logger('arr: ', [
	...
	async listTodo(rescanInterval) {
		...
		let bloc = false;
		res[0].forEach(async el => {
			let addToArr = true;
			arr.forEach(async ele => {
				if (el.peer_id === ele) {
					addToArr = false;
				}
				if (addToArr && ele === arr[arr.length - 1].peer_id) {
					arr.push(el.peer_id);
					await this.blockPeer(el.peer_id, BLOCK_REASONS.UNREACHABLE);
					logger(
						'Blocked peer with ID ' + el.peer_id + '. Reason: Unreachable!'
					);
					bloc = true;
				}
			});
			if (el === res[res.length - 1]) {
				if (bloc) {
					...
					const res2 = await this.db.execute(''
						...
						+'WHERE blocked = ? AND TIMESTAMPDIFF(MINUTE, last_scanned, NOW()) < ?'
					,[BLOCK_REASONS.UNREACHABLE, def.blockedPeersCooldown*3600000]);
					if (res2.length <= 0) {
						// Signal Up to Date
						exit(200);
					} else {
						res2[0].forEach(async elem => {
							await this.blockPeer(elem.peer_id, BLOCK_REASONS.NOT_BLOCKED);
							logger(
								'Unblocked peer with ID ' +
									elem.peer_id +
									'. Reason: Cooldown time exceeded!'
							);
							if (elem.peer_id === res2[res2.length - 1].peer_id) {
								// Signal Peers Corrected
								exit(240);
							}
						});
