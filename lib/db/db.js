  ...
  async queryDB(table, prop, id, extra, extraData, order, orderProp, show='*') {
	...
	async updateIP(id, ip, old, error, isOld){
		...
		if(!error){
			...
			hash = require('crypto').createHash('md5').update(id+','+old).digest('hex');
			let blocked = BLOCK_REASONS.NOT_BLOCKED;
			if(old === 'NOT FOUND'){
				blocked = BLOCK_REASONS.ILLEGAL_ADDRESS;
				const oldIp = await this.queryDB(
					'checks',
					'hash',
					hash,
					'',
				...
				if(oldIp[0]){
					if(oldIp[0].ip && typeof oldIp[0].ip === 'string' && oldIp[0].ip !== 'NOT FOUND'){
						old = oldIp[0].ip;
						ip = old;
						blocked = BLOCK_REASONS.NEW_IP;
					}
				}
			}
			if(!isOld){
				query ='INSERT checks (hash, blocked, peer_id, ip, last_scanned) VALUES (?,?,?,?,NOW()) '
							+'ON DUPLICATE KEY UPDATE '
								...
								+'last_scanned = NOW(), '
								+'ip = ?';
				arr = [hash, blocked, id, ip, id, blocked, hash, old];
			} else {
				oldHash = require('crypto').createHash('md5').update(id+','+null).digest('hex');
				query =  'UPDATE checks '
								+'SET peer_id = ?, '
										+'blocked = ?, '
										+'hash = ?, '
										...
										+'ip = ? '
								+'WHERE hash = ?';
				arr = [id, blocked, hash, old, oldHash];
			}
		} else {
