...
const domainCheck = async (peer) => {
  ...
  if(typeof peer.ip !== 'string' || peer.ip === 'NOT FOUND'){
    ...
    await db.checks.updateLocData({city: 'N/A', country: 'N/A', region: 'N/A', lat: 'N/A', long: 'N/A'}, peer.id, peer.ip);
...
const locationHandler = async (peer) => {
  ...
  const resId = await db.checks.updateLocData(resLoc, peer.id, peer.ip);
  return;
}
