...
const domainCheck = async (peer, isOld) => {
	...
	if (typeof peer.ip !== 'string' || peer.ip === 'NOT FOUND') {
		await db.checks.updateIP(peer.id, 'NOT FOUND', null, null, isOld);
		// Update SSL status code to INVALID(1)
		hash = await db.checks.updateSSLStatus(peer.id);
		// Update location info
		await db.checks.updateLocData(
			{ city: 'N/A', country: 'N/A', region: 'N/A', lat: 'N/A', long: 'N/A' },
			peer,
			hash
		);
		...
		await db.checks.updateIP(peer.id, peer.ip, null, null, isOld);
		let resSSL;
		try {
...
const locationHandler = async (peer, hash) => {
	...
	await db.checks.updateLocData(resLoc, peer, hash);
	return;
};
// Handle Public Wallet checks
const publicWalletHandler = async peer => {
	// Save Public Wallet checks
	const resPub = await self.walletAPICheck(
		utils.normalizeAPIPort(peer.address)
	);
...
exports.walletAPICheck = async domain => {
	...
	try {
		await brs.callBRS(domain, brs.BRS_API_REQUESTS.TIME);
		ob = {
			isPublicAPI: true,
			apiPort: port
		};
